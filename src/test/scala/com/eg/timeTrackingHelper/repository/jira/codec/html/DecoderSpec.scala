...
class DecoderSpec extends AnyFlatSpec with Matchers with EitherValues {
  ...
  "DecoderSpec" should "correctly parse the list of TicketStatusStatistics" in {
    val expectedResult = List(
      TicketStatusStatistics(
        ...
        Map(
          TicketStatus("In Progress") -> List(
            DatePeriod(
              simpleDateFormat.parse("08/Jan/20 5:54 PM"),
              simpleDateFormat.parse("08/Jan/20 6:05 PM")
            ),
            DatePeriod(
              simpleDateFormat.parse("09/Jan/20 2:11 PM"),
              simpleDateFormat.parse("29/Jan/20 2:26 PM")
            ),
            DatePeriod(
              simpleDateFormat.parse("29/Jan/20 2:32 PM"),
              simpleDateFormat.parse("29/Jan/20 2:52 PM")
            )
          ),
      ...
      TicketStatusStatistics(
        ...
        Map(
          TicketStatus("Open") -> List(
            DatePeriod(
              simpleDateFormat.parse("12/Aug/19 12:59 PM"),
              simpleDateFormat.parse("25/Nov/19 5:20 PM")
            ),
            DatePeriod(
              simpleDateFormat.parse("27/Nov/19 11:12 AM"),
              simpleDateFormat.parse("02/Jan/20 11:24 AM")
            ),
            DatePeriod(
              simpleDateFormat.parse("06/Jan/20 5:40 PM"),
              simpleDateFormat.parse("08/Jan/20 5:54 PM")
            )
          ),
    ...
    loadResource("decoder/correct_list.html")
      .toEntity[List[TicketStatusStatistics]]
  ...
  it should "return HtmlDeserializationError -> PathNotFoundException if element not found" in {
    ...
    loadResource("decoder/incorrect_path_list.html")
      .toEntity[List[TicketStatusStatistics]]
      .left
      .value should matchPattern { case HtmlDeserializationError(PathNotFoundException(_)) =>
    }
  }
  ...
  it should "return HtmlDeserializationError -> ParseException if date is in the wrong format" in {
    ...
    loadResource("decoder/incorrect_date_list.html")
      .toEntity[List[TicketStatusStatistics]]
      .left
      .value should matchPattern { case HtmlDeserializationError(_: ParseException) =>
    }
  }
  ...
  it should "correctly parse the TicketStatusStatistics" in {
    ...
    loadResource("decoder/correct_entity.html")
      .toEntity[TicketStatusStatistics]
      .value should be(expectedResult)
  }
