...
class MeetingServiceSpec extends AnyFlatSpec with MockitoSugar with Matchers with EitherValues {
  ...
  private val keywordMapping = KeywordMapping(
    defaultTicket,
    Map((ticket1, Set("test-1", "test-2", "test-3")), (ticket2, Set("test-4", "test-5", "test-6")))
  )
  ...
  "MeetingService.getTicketIdBySubject" should "return correctly ticketId based on letter subject" in {
    meetingService.getTicketIdBySubject(
      "A small discussion about test-2 and test-5".some
    ) should be(TicketId(ticket1).some)
    meetingService.getTicketIdBySubject("A small discussion about test-3".some) should be(
      TicketId(ticket1).some
    )
    meetingService.getTicketIdBySubject(
      "A small discussion about test-5 and test-2".some
    ) should be(TicketId(ticket1).some)
    meetingService.getTicketIdBySubject("A small discussion about test-4".some) should be(
      TicketId(ticket2).some
    )
  }

  it should "return the ticket based on a partial match" in {
    meetingService.getTicketIdBySubject("A small discussion about test-25".some) should be(
      TicketId(ticket1).some
    )
    meetingService.getTicketIdBySubject("A small discussion about test-43".some) should be(
      TicketId(ticket2).some
    )
  }

  it should "return default ticketId" in {
    meetingService.getTicketIdBySubject("A small discussion about vacations".some) should be(
      TicketId(defaultTicket).some
    )
  ...
  "MeetingService.getLogEntityByMeeting" should "return LogEntity by Meeting" in {
    ...
    val meeting = Meeting(subject, isTakePlace = true, start, end, 1 hour)
    val logEntity = WorklogEntity(start, end, TicketId(defaultTicket), ActivityType.Major, subject)
  ...
  "MeetingService.getMeetingsLogEntity" should "return Map[LocalDate, List[WorklogEntity]] with upcoming meetings" in {
    ...
    result.get(start.plusDays(1)) should be(
      Some(
        List(
          WorklogEntity(
            meetingStart.plusDays(1),
    ...
    result.get(start.plusDays(2)) should be(
      Some(
        List(
          WorklogEntity(
            meetingStart.plusDays(2),
  ...
  it should "return exception(Negative scenario)" in {
    ...
    meetingService.getMeetingsLogEntity(datePeriod).attempt.unsafeRunSync() should be(
      unknownException.asLeft
    )
  }
}
