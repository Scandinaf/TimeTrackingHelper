  ...
  override def handle(exception: Throwable): IO[Unit] =
    exception match {
      case _: UnauthorizedException =>
        IO(println("Failed to login, please check your login and password."))
      case _: ServerNotFoundException =>
        IO(println("Couldn't find the mail server, please check the configuration."))
      case _: DateTimeParseException =>
        IO(println("We expect the date to be entered in the following format - YYYY-MM-DD."))
