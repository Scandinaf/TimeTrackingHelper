...
class TimeTrackingHelperRoutes(
  ...
  val routes: HttpRoutes[IO] = AutoSlash(HttpRoutes.of[IO] {
    ...
    case req @ GET -> Root / "timeTrackingHelper" / "form" =>
      StaticFile
        .fromResource("html/TimeTrackingForm.html", blocker, Some(req))
        .getOrElseF(NotFound())
    ...
    case req @ POST -> Root / "timeTrackingHelper" / "form" =>
      (for {
        ...
        IO(logger.error("The request couldn't be completed correctly", throwable)) *> handle(
          throwable
        )
      )
  ...
  private def decodeUrlForm(req: Request[IO]): IO[Response[IO]] =
    req.decode[UrlForm] { form =>
      getAccessToken(form).flatMap {
        ...
        case accessToken =>
          tokenRef.set(accessToken) *> MovedPermanently(Location(uri"/timeTrackingHelper/form"))
      }
    }

  private def getAccessToken(urlForm: UrlForm): IO[Option[String]] =
    IO(urlForm.get("access_token").toList.filterNot(_.isBlank) match {
      case Nil  => None
...
object TimeTrackingHelperRoutes {
  def apply(
    ...
    blocker: Blocker,
    timeTrackingService: TimeTrackingService
  )(implicit context: ContextShift[IO]): TimeTrackingHelperRoutes =
