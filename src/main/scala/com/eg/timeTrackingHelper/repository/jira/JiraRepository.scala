...
class JiraRepository(config: JiraConfig)(implicit contextShift: ContextShift[IO])
    extends Repository {
  ...
  private val client: JiraSingleTenantClient[Id] = JiraSingleTenantClient(
    BasicAuthentication(config.url.toString, config.userName, config.password)
  )

  def getTicketStatusStatistics(ticketId: TicketId): IO[List[TicketStatusStatistics]] =
    for {
      content <- Async[IO].async[String] { io =>
        io(
          client.getResourceHtml(
            ...
            params =
              Map("page" -> "ru.andreymarkelov.atlas.plugins.datacollector:user-status-panel"),
  ...
  def getTickets(ticketIds: Set[TicketId], fields: Option[Set[String]] = None): IO[SearchResults] =
    Async[IO].async(
      _(client.search(jql = s"id in (${ticketIds.map(_.value).mkString(",")})", fields = fields))
    )

  def getTicket(ticketId: TicketId, fields: List[String] = List("*all")): IO[Issue] =
    ...
    Async[IO].async { io =>
      ...
      io(
        client.search(
          ...
          jql = s"""assignee was currentUser()
               | $periodParameters
               | AND
  ...
  private def dateToPeriodQueryParameters(datePeriod: DatePeriod): String = {
...
object JiraRepository {
  def apply(config: JiraConfig)(implicit contextShift: ContextShift[IO]): JiraRepository =
