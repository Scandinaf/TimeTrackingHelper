...
import com.eg.timeTrackingHelper.repository.jira.codec.exception.{
  HtmlDeserializationError,
...
private[jira] object Decoder {
  ...
  private[html] sealed trait TicketStatusStatisticsDecoder {
    ...
    private def getUser(htmlElement: Element): Either[PathNotFoundException, User] =
      tryGetUser(htmlElement)
        .orElse(tryGetUserFromAlternativeSource(htmlElement))
        .map(_.asRight)
        .getOrElse(
          PathNotFoundException(
    ...
    private def tryGetUserFromAlternativeSource(htmlElement: Element): Option[User] =
      (htmlElement >?> element(CssSelector.alternativeSourceUser))
        .map(element => User(element.text, element.text))

    private def tryGetUser(htmlElement: Element): Option[User] =
      (htmlElement >?> element(CssSelector.user))
        .map(element => User(element.attr("rel"), element.text))
    ...
    private def getTicketStatus(htmlElement: Element): Either[PathNotFoundException, TicketStatus] =
      (htmlElement >?> text(CssSelector.ticketStatus))
        .fold(PathNotFoundException(CssSelector.ticketStatus).asLeft[TicketStatus])(
    ...
    private def parseDateTimePeriod(strings: List[String]): Either[Throwable, List[DatePeriod]] =
      strings
        .mapFilter(_.split(regex) match {
        ...
        .traverse(period =>
          Try(
            DatePeriod(simpleDateFormat.parse(period._1), simpleDateFormat.parse(period._2))
          ).toEither
        )

    protected def parse(htmlElement: Element): Either[Throwable, TicketStatusStatistics] =
      for {
        user <- getUser(htmlElement)
        statuses <- parseStatuses(htmlElement)
      } yield TicketStatusStatistics(user, statuses)
  ...
  implicit val listTicketStatusStatisticsDecoder =
    new Decoder[List[TicketStatusStatistics]] with TicketStatusStatisticsDecoder {
      ...
      override def parse(
        content: String
      ): Either[HtmlDeserializationError, List[TicketStatusStatistics]] =
        Try(browser.parseString(content) >> elementList(mainSelector) match {
          case Nil  => PathNotFoundException(mainSelector).asLeft
          case list => list.traverse(parse)
        }).toEither.flatten
          .leftMap(cause => HtmlDeserializationError(cause))
    }
  ...
  implicit val ticketStatusStatisticsDecoder: Decoder[TicketStatusStatistics] =
    new Decoder[TicketStatusStatistics] with TicketStatusStatisticsDecoder {
      ...
      override def parse(
        content: String
      ): Either[HtmlDeserializationError, TicketStatusStatistics] =
        Try(parse(browser.parseString(content).body)).toEither.flatten
          .leftMap(cause => HtmlDeserializationError(cause))
    }
