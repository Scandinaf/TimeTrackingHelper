...
private[service] class TicketService(
  ...
  private def getLogEntitiesByTicket(ticket: Issue): IO[List[WorklogEntity]] = {
    ...
    ticket.changelog.map(
      _.histories.foldLeft(
        TicketState().asRight[Throwable]
      )(
        (ticketState, history) =>
          ticketState.flatMap(
            mergeTicketState(
              _,
              getLogEntitiesByHistoryItems(history.items)
            )(
              ticketId,
              history.created
            )
  ...
  private def ticketStatusStatisticsToWorklogEntity(
    ...
    ticketStatusStatistics
      .statuses
      .filterKeys(ticketStatuses.contains(_))
      .toList
      .flatMap(tuple => {
  ...
  private def getLogEntitiesByHistoryItems(
                                            historyItems: List[HistoryItem]
                                          ): TicketState =
    historyItems.foldLeft(
      TicketState()
    )(
      (ticketState, historyItem) =>
        historyItem match {
