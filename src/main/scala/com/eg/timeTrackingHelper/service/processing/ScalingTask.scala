...
private[processing] trait ScalingTask extends BaseTask {
  ...
  private def makeScaling(state: WorkLogTransformationState): WorkLogTransformationState =
    state.payloads.foldLeft(state.copy(payloads = List.empty))((state, element) =>
      if (state.totalDuration < workHoursLimit) {
        ...
        state.copy(
          state.payloads :+ element.copy(payload =
            element.payload.copy(timeSpentSeconds = element.payload.timeSpentSeconds + scaleTime)
          ),
