...
import com.eg.timeTrackingHelper.configuration.ApplicationConfig.applicationSettings.{minTaskTime, workHoursLimit}

private[processing] trait CalculateCompletedMajorTicketsTask extends BaseTask {

  protected def calculateCompletedMajorTickets(
                                                worklogs: List[WorklogPayloadWithTicket]
                                              ) =
    (state: WorkLogTransformationState) =>
      worklogs.foldLeft(state)(
        (state, element) =>
  ...
  private def calculateState(
    ...
    val taskDuration = calculateTaskDuration(
      element.payload.timeSpentSeconds,
      state.totalDuration
    )
