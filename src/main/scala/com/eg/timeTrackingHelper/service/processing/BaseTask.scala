...
import com.eg.timeTrackingHelper.configuration.ApplicationConfig.applicationSettings.{minTimeFrame, workHoursLimit}
...
private[processing] trait BaseTask {
  ...
  protected def buildWorklogWithTicket(
  ...
  protected def calculateTaskDuration(
                                       duration: Int,
                                       totalDuration: Int
                                     ): Int =
    checkOverflow(
      DateTimeHelper.scaleToTimeFrame(
        duration,
        minTimeFrame
      ),
      totalDuration
