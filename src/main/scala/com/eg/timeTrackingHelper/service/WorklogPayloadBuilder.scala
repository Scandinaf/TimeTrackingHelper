...
import com.eg.timeTrackingHelper.service.model.{HierarchicalWorklog, WorklogEntity, WorklogPayloadWithTicket}
...
private[service] trait WorklogPayloadBuilder
  ...
  protected val dateTimeFormatter = DateTimeFormatter.ofPattern("yyyy-MM-dd'T'hh:mm:ss.SSSZ")
  ...
  protected def toWorklogPayloads(
    ...
    Stream.emits(datePeriod.toListLocalDate).covary[IO]
      .mapAsyncUnordered(2)(
        localDate =>
          IO((
            localDate,
            toHierarchicalWorklogs(
              WorklogEntityMap.getOrElse(
                localDate,
                List.empty
              )
            )
      ...
      tuple =>
        IO(buildWorkLogPayLoads(tuple._1, tuple._2))
    )
  ...
  protected def buildWorkLogPayLoads(
    ...
    implicit val startedDateTime = localDate.atStartOfDay()
      .plusHours(10)
      .atZone(DateTimeHelper.defaultZoneId)
      .format(dateTimeFormatter)

    val map = hierarchicalWorklog.major.groupBy(
      entity =>
        TaskState.buildKey(
          entity.start.toLocalDate,
          entity.end.toLocalDate,
          localDate
        )
    )
  ...
  protected def merge(
                       list: List[WorklogPayloadWithTicket]
                     ): List[WorklogPayloadWithTicket] =
    list.groupBy(_.ticketId).values.toList.map {
      case head :: Nil => head
      case list@head :: _ =>
        head.copy(
          payload = head.payload.copy(
            comment = list.map(
              _.payload.comment
            ).flatten
              .some
  ...
  private def getCompletedMajorTickets(
                                        ...
                                        implicit startDateTime: String
                                      ): List[WorklogPayloadWithTicket] =
    merge(
      worklogs.map(
        worklogEntity =>
          buildWorklogWithTicket(
            worklogEntity,
            worklogEntity.duration.toSeconds.toInt
          )
      )
